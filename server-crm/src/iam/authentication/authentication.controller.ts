...
export class AuthenticationController {
  ...
  @Throttle({ default: { limit: 25, ttl: 60000 } }) // 25 requests per minute for sign-up
  @Post('sign-up')
      ...
      response.cookie('refreshToken', refreshToken, {
        ...
        if (error.message && error.message.startsWith('{')) {
          try {
            ...
            // Если не удалось распарсить, возвращfаем обычную ошибку
          }
        }
        throw error;
      }
  ...
  async inviteUser(@Body() inviteUserDto: InviteUserDto) {
    await this.authService.inviteUser(inviteUserDto.email, inviteUserDto.role);
  ...
  async validateInviteToken(@Query('token') token: string) {
    ...
    const isValid = await this.authService.validateInviteToken(token);
    return { valid: isValid };
  }

  @Auth(AuthTypeEnum.None)
  @Get('get-invite-email')
  async getInviteEmail(@Query('token') token: string) {
    if (!token) {
      throw new BadRequestException('Token is required');
    }
    const email = await this.authService.getEmailFromInviteToken(token);
    if (!email) {
  ...
  @Auth(AuthTypeEnum.None)
  @Throttle({ default: { limit: 5, ttl: 60000 } }) // 5 requests per minute for sign-up-with-token
  @HttpCode(HttpStatus.OK)
  @Post('sign-up-with-token')
  async signUpWithToken(@Body() signUpWithTokenDto: SignUpWithTokenDto) {
    // Validate that passwords match
    if (signUpWithTokenDto.password !== signUpWithTokenDto.confirmPassword) {
      throw new BadRequestException('Passwords do not match');
    }

    return this.authService.signUpWithToken({
      token: signUpWithTokenDto.token,
      email: signUpWithTokenDto.email,
      password: signUpWithTokenDto.password,
      name: signUpWithTokenDto.name,
      role: signUpWithTokenDto.role,
      phones: signUpWithTokenDto.phones,
    });
  }
}
